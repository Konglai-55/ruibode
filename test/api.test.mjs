...
test('管理员可搜索审核与已有战队，已结束赛事自动移出审核工作台但保留历史', async () => {
  ...
  try {
    ...
    assert.equal(allUsers.payload.users[0].admin_level, 'super', '管理用户列表应将最高管理员置顶');
    ...
    assert.match(blockedSelfDemotion.payload.error, /不能修改当前登录管理员自己的权限/);
    ...
    assert.equal(app.db.prepare('SELECT admin_level FROM users WHERE id=?').get(team.user_id).admin_level, 'mid');
    ...
    const middleAdmin = client(app.base);
    await middleAdmin.login('demo@ruibude.local', 'Demo123!');
    const blockedSuperChange = await middleAdmin.request(`/api/admin/users/${currentAdmin.id}/role`, { method: 'POST', body: JSON.stringify({ admin_level: 'mid' }) });
    assert.equal(blockedSuperChange.response.status, 403, '中级管理员不能撤销最高管理员权限');
    const madeSuper = await admin.request(`/api/admin/users/${team.user_id}/role`, { method: 'POST', body: JSON.stringify({ admin_level: 'super' }) });
    assert.equal(madeSuper.response.status, 200, '最高管理员可以提拔中级管理员');
    assert.equal(app.db.prepare('SELECT admin_level FROM users WHERE id=?').get(team.user_id).admin_level, 'super');
    const restoredMiddle = await admin.request(`/api/admin/users/${team.user_id}/role`, { method: 'POST', body: JSON.stringify({ admin_level: 'mid' }) });
    assert.equal(restoredMiddle.response.status, 200, '最高管理员可以将其他最高管理员降为中级管理员');
    const promotedDetail = await admin.request(`/api/admin/users/${team.user_id}`);
    assert.equal(promotedDetail.payload.user.role, 'admin');
    assert.equal(promotedDetail.payload.user.admin_level, 'mid');
