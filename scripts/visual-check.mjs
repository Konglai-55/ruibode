...
for (const profile of [
  ...
  const homeInfo = await page.evaluate(() => {
    ...
    return {
      ...
      activeHeroLogoOnly: document.querySelector('.home-hero-slide.is-active')?.classList.contains('is-logo-only'),
      activeHeroObjectFit: getComputedStyle(document.querySelector('.home-hero-slide.is-active img')).objectFit,
      activeHeroBackground: getComputedStyle(document.querySelector('.home-hero-slide.is-active')).backgroundColor,
  ...
  checks.push({ profile: profile.name, ...homeInfo, blobVideoState, overflow: homeInfo.scrollWidth > homeInfo.clientWidth + 1, valid: homeInfo.title === '准备好开启你的机器人竞技之旅了吗？' && homeInfo.programCount === 3 && homeInfo.dotCount === 5 && homeInfo.activeDot === '2' && homeInfo.activeHero === '/assets/home/hero-ruibude-logo.png' && homeInfo.activeHeroLogoOnly && homeInfo.activeHeroObjectFit === 'contain' && homeInfo.activeHeroBackground === 'rgb(29, 61, 102)' && ratioValid && homeInfo.heroAligned && homeInfo.firstHeroLoaded && homeInfo.firstHeroSrc === '/assets/home/hero-recf.png' && !homeInfo.firstHeroLogoOnly && homeInfo.firstHeroObjectFit === 'cover' && homeInfo.programImagesLoaded && homeInfo.homeHref === '#/home' && featuredVideoValid && homeInfo.heroHrefs.join('|') === expectedHeroHrefs.join('|') && homeInfo.heroActions.join('|') === expectedHeroActions.join('|') && homeInfo.partnerHrefs.join('|') === expectedPartnerHrefs.join('|') && homeInfo.partnerImageSrcs.join('|') === expectedPartnerImageSrcs.join('|') && homeInfo.platformHrefs.join('|') === expectedPlatformHrefs.join('|') && homeInfo.logoHrefs.join('|') === expectedLogoHrefs.join('|') && homeInfo.scrollWidth <= homeInfo.clientWidth + 1 });
