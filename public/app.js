...
function eventContactText(event) {
  return event?.contact_info || [event?.contact_name, event?.contact_phone].filter(Boolean).join(' · ') || '—';
}
...
async function adminEventFormPage(id) {
  if(!requireAdmin())return; let event={groups:[...DEFAULT_TEAM_GROUPS],status:'published',published_at:new Date().toISOString(),contact_name:'小周老师',contact_phone:'13761393714',contact_info:'小周老师\n13761393714',payee:'上海瑞卜德教育科技有限公司',account_no:'153189255',bank_code:'',bank_name:'中国民生银行股份有限公司上海凯旋支行',allow_volunteer:false,allow_spectator:false,refund_deadline_days:10};
