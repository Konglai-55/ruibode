...
function renderHeader() {
  ...
  </div><nav class="mobile-drawer" aria-label="移动端导航"><a class="${homeActive ? 'active' : ''}" href="#/home"${homeActive ? ' aria-current="page"' : ''}>${icon('home')}首页</a><div class="mobile-nav-group"><strong class="mobile-nav-title">活动报名</strong>${mobileActivityLinks}</div><a class="${active('/rules')}" href="#/rules"${path.startsWith('/rules') ? ' aria-current="page"' : ''}>${icon('file')}赛事规则</a><a class="${active('/about')}" href="#/about"${path.startsWith('/about') ? ' aria-current="page"' : ''}>${icon('info')}关于我们</a>${accountLinks}${user ? `<button class="logout" type="button" data-action="logout">${icon('logout')}退出登录</button>` : `<a href="#/login">${icon('user')}登录</a><a href="#/register">${icon('mail')}注册</a>`}</nav><nav class="mobile-bottom-nav" aria-label="移动端快捷导航"><a class="${homeActive ? 'active' : ''}" href="#/home"${homeActive ? ' aria-current="page"' : ''}>${icon('home')}<span>首页</span></a><a class="${path.startsWith('/events') ? 'active' : ''}" href="#/events"${path.startsWith('/events') ? ' aria-current="page"' : ''}>${icon('calendar')}<span>赛事中心</span></a><button class="${path.startsWith('/account') || path.startsWith('/admin') ? 'active' : ''}" type="button" data-action="toggle-mobile-menu" aria-expanded="false" aria-label="打开个人中心">${icon('user')}<span>个人中心</span></button></nav>`;
}
