...
function renderHeader() {
  ...
  </div><nav class="mobile-drawer" aria-label="移动端导航"><a class="${homeActive ? 'active' : ''}" href="#/home"${homeActive ? ' aria-current="page"' : ''}>${icon('home')}首页</a><div class="mobile-nav-group"><strong class="mobile-nav-title">活动报名</strong>${mobileActivityLinks}</div><a class="${active('/rules')}" href="#/rules"${path.startsWith('/rules') ? ' aria-current="page"' : ''}>${icon('file')}赛事规则</a><a class="${active('/about')}" href="#/about"${path.startsWith('/about') ? ' aria-current="page"' : ''}>${icon('info')}关于我们</a>${accountLinks}${user ? `<button class="logout" type="button" data-action="logout">${icon('logout')}退出登录</button>` : `<a href="#/login">${icon('user')}登录</a><a href="#/register">${icon('mail')}注册</a>`}</nav><nav class="mobile-bottom-nav" aria-label="移动端快捷导航"><a class="${homeActive ? 'active' : ''}" href="#/home"${homeActive ? ' aria-current="page"' : ''}>${icon('home')}<span>首页</span></a><a class="${path.startsWith('/events') ? 'active' : ''}" href="#/events"${path.startsWith('/events') ? ' aria-current="page"' : ''}>${icon('calendar')}<span>赛事中心</span></a><a class="${path === '/account' ? 'active' : ''}" href="#/account"${path === '/account' ? ' aria-current="page"' : ''}>${icon('user')}<span>个人中心</span></a></nav>`;
}
...
function mobileAccountCenterPage() {
  const user = state.user;
  const item = (href, iconName, label, description) => `<a class="mobile-account-link" href="${href}">${icon(iconName)}<span><strong>${label}</strong><small>${description}</small></span>${icon('chevron', 17)}</a>`;
  const actions = user
    ? `${item('#/rules','file','赛事规则','查看三大赛项规则手册')}${item('#/about','info','关于我们','了解上海瑞卜德教育')}${item('#/account/teams','users','战队管理','创建和维护参赛战队')}${item('#/account/registrations','trophy','我的比赛','查看报名及审核状态')}${user.role === 'admin' ? item('#/admin','dashboard','管理后台','赛事、用户与报名审核') : ''}<button class="mobile-account-link mobile-account-logout" type="button" data-action="logout">${icon('logout')}<span><strong>退出登录</strong><small>安全退出当前账号</small></span>${icon('chevron', 17)}</button>`
    : `${item('#/rules','file','赛事规则','查看三大赛项规则手册')}${item('#/about','info','关于我们','了解上海瑞卜德教育')}<a class="mobile-account-link" href="#/login"><span><strong>登录账号</strong><small>登录后管理战队和报名</small></span>${icon('chevron', 17)}</a><a class="mobile-account-link" href="#/register"><span><strong>注册账号</strong><small>创建你的报名账号</small></span>${icon('chevron', 17)}</a>`;
  app.innerHTML = `<section class="mobile-account-page"><div class="container"><div class="mobile-account-heading"><span class="record-label">ACCOUNT CENTER</span><h1>个人中心</h1><p>${user ? '管理你的赛事报名与账户信息' : '登录后管理你的赛事报名与账户信息'}</p></div>${user ? `<div class="mobile-account-profile">${user.avatar_url ? `<img src="${safeUrl(user.avatar_url)}" alt="" width="52" height="52">` : `<span class="mobile-account-avatar">${escapeHtml((user.nickname || user.email)[0].toUpperCase())}</span>`}<div><strong>${escapeHtml(user.nickname || '报名用户')}</strong><small>${escapeHtml(user.email)}</small></div></div>` : ''}<div class="mobile-account-links">${actions}</div></div></section>`;
}
...
document.addEventListener('click',async(event)=>{
  ...
  if(action==='toggle-mobile-menu'){const drawer=$('.mobile-drawer',header);drawer.classList.toggle('open');const open=drawer.classList.contains('open');$$('[data-action="toggle-mobile-menu"]',header).forEach((button)=>{button.setAttribute('aria-expanded',String(open));button.setAttribute('aria-label',open?'关闭个人中心':'打开个人中心');});}
...
async function render() {
  ...
  try{
    ...
    else if(path==='/account')mobileAccountCenterPage();
