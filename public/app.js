...
function mobileAccountCenterPage() {
  ...
  const actions = user
    ? `${item('#/about','info','关于我们','了解上海瑞卜德教育')}${item('#/account/teams','users','战队管理','创建和维护参赛战队')}${item('#/account/registrations','trophy','我的比赛','查看报名及审核状态')}${user.role === 'admin' ? item('#/admin','dashboard','管理后台','赛事、用户与报名审核') : ''}<button class="mobile-account-link mobile-account-logout" type="button" data-action="logout">${icon('logout')}<span><strong>退出登录</strong><small>安全退出当前账号</small></span>${icon('chevron', 17)}</button>`
    : `${item('#/about','info','关于我们','了解上海瑞卜德教育')}<a class="mobile-account-link" href="#/login"><span><strong>登录账号</strong><small>登录后管理战队和报名</small></span>${icon('chevron', 17)}</a><a class="mobile-account-link" href="#/register"><span><strong>注册账号</strong><small>创建你的报名账号</small></span>${icon('chevron', 17)}</a>`;
